...
| 화면 ID | CON01-L |
| 화면명 | 도급계약 목록 |
| URL | `/contracts` (기본) / `/contracts?tab=list` |
| 화면 유형 | L (List) |
...
| 요소 | 위치 | 액션 | 조건·비고 |
|------|------|------|---------|
| 탭 `[도급계약 목록]` | 상단 탭 | 현재 화면 | 활성 |
| 탭 `[수급업체 목록]` | 상단 탭 | CON02-L 이동 (`?tab=subcontractors`) | |
| 탭 `[협의체 운영]` | 상단 탭 | CON03-L 이동 (`?tab=council`) | |
| 탭 `[TBM]` | 상단 탭 | CON09-L 이동 (`?tab=tbm`) | 도급공사 계약 1건 이상 시 노출 |
| 탭 `[설정]` | 상단 탭 | CON10-S 이동 (`?tab=settings`) | CEO/GM/SHM만 |
...
CON01-L
   ...
   ├─ 탭 → CON02-L / CON03-L / CON09-L / CON10-S
