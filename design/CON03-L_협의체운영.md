...
> 모듈 내 다른 화면(도급계약 / 수급업체 / TBM / 도급관리 설정)으로의 이동은 좌측 사이드 메뉴(LNB)로 처리.
...
| 회의록 아이콘 | 목록 | 회의록 새 창 다운로드 | 첨부 있을 때 |
...
| v1.1 | 2026-05-12 | 모듈 탭 제거 → LNB 사이드 메뉴로 이동. URL `?tab=council` → `/council` |
