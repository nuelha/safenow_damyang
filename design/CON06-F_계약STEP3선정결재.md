...
| `[전담인력 화면으로]` | 안전관리자 미달 모달 | ORG03-V 이동 (`?department_id=`) | 미달 시 모달에 노출 |
...
본 화면 진입 시 실시간 재검증 (사용자 ORG03-V에서 선임 후 복귀할 수 있음)
```
...
| BLOCKED 상태에서 결재 요청 시도 | "결재 차단 상태입니다. 안전관리자를 추가 선임해주세요" | 인라인 + ORG03-V 링크 |
