...
| 화면 ID | RSK03-L |
| 화면명 | 개선조치 (위험성평가 출처) |
| URL | `/risk-assessment/improvements` |
| 화면 유형 | L (List) |
| 접근 권한 | SHM+ (열람·진행) / 데이터 스코프 적용 |

## 2. 역할

IMP01-L (개선조치 모듈) 의 source_type='risk_assessment' 필터링 뷰. 위험성평가에서 자동 생성된 개선조치만 본 화면에서 조회.

> 본 화면은 RSK 모듈의 LNB 항목 중 하나. 실제 개선조치 CRUD는 IMP 모듈(Phase 2)에서 담당.
...
> 모듈 내 다른 화면(현황 / 평가 목록 / 위험성평가 설정)으로의 이동은 좌측 사이드 메뉴(LNB)로 처리.
...
┌──────────────────────────────────────────────────────────┐
│ ● 개선조치                                                │
│   위험성평가에서 자동 생성된 개선조치 진행 관리             │
│                                                          │
│ 요약 카드                                                  │
│ 기한초과 N / 조치중 N / 예정 N / 완료 N                     │
│                                                          │
│ 필터: [부서▼][담당자▼][상태▼][기한▼][검색] [초기화]        │
│                                                          │
...
| v1.1 | 2026-05-12 | 화면명 "개선조치 탭" → "개선조치". 모듈 탭 제거 → LNB 사이드 메뉴로 이동. URL `?tab=improvements` → `/improvements` |
