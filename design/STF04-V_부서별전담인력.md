# STF04-V — 부서별 전담인력 현황 (구 ORG03-V)

> 화면 ID: **STF04-V** (구 ORG03-V — PRD-DAMYANG-001 v1.4에서 이전)
> 모듈: STF (안전보건 인력)
> SFR: **SFR-006 (조직) + SFR-010 (안전보건 인력) 공동 매핑**
> 버전: v1.1 | 작성일: 2026-05-11 (v1.0) / 2026-05-14 (v1.1)
> 관련 문서: SRS-001 §3 SFR-006·010, DB-001 §6.1 dedicated_personnel / §2.4 target_assignees / §6.2 user_qualifications, IA-001 v1.4, PRD-DAMYANG-001

> ※ **v1.1 변경 (2026-05-14)**: PRD-001(조직·인력 모듈 통합) + PRD-002(GNB 그룹핑) 반영.
> - 화면 ID: ORG03-V → **STF04-V**
> - URL: `/organization/dedicated` → **`/staff/dedicated`**
> - 모듈: ORG (조직) → **STF (안전보건 인력)**
> - SFR: SFR-006 → **SFR-006 + SFR-010 공동 매핑**
> - 진입 경로: GNB > 전담조직 > 전담인력 (v1.0) → **GNB 7. 기반 관리 > 안전보건 인력 > 부서별 전담인력** (v1.4 IA)
> - 컨텐츠 자체는 v1.0과 동일 (부서별 안전관리자·보건관리자·관리감독자·산업보건의 선임 관리).
...
| 항목 | 내용 |
|------|------|
| 화면 ID | STF04-V (구 ORG03-V) |
| 화면명 | 부서별 전담인력 현황 |
| URL | `/staff/dedicated` (구 `/organization/dedicated`) |
| 화면 유형 | V (View) |
...
- GNB **기반 관리** > LNB **안전보건 인력 > 부서별 전담인력** (v1.4 IA)
- ORG01-V 우측 패널 [부서별 전담인력 관리]
- ORG01-V 우측 패널 안전·보건 충족 행 클릭 (해당 역할 카드로 scroll)
- STF01-V 부서별 행 클릭 (해당 부서 카드로 scroll)
- CON06-F 안전관리자 미달 모달 [전담인력 화면으로]
- 새올 포틀릿 알림 (안전관리자 자격 미보유 강제 선임 등) 링크

> 모듈 내 다른 화면(STF01-V 전체 현황 / STF02-V 공사별 / STF03-S 설정)으로의 이동은 좌측 사이드 메뉴(LNB)로 처리.
...
| `[선임]` | 미충족 카드 우측 | STF04-M 모달 (선임 모드) | SHM+ |
| `[수정]` | 충족 카드 우측 | STF04-M 모달 (수정 모드, prefill) | SHM+ |
| `[+ 선임하기]` | 담당자 미지정 카드 | STF04-M 모달 (선임 모드) | SHM+ |
...
### 7.1 자격증 보유 검증 (STF04-M 선임 시점에 수행)
...
| 버전 | 날짜 | 내용 |
|------|------|------|
| v1.0 | 2026-05-11 | 초안 작성 (ORG03-V). dedicated_personnel(부서 단위)와 target_assignees(관리대상 단위) 분리 운영 명시 |
| v1.1 | 2026-05-12 | 모듈 탭 제거 → LNB 사이드 메뉴로 이동. URL `?tab=dedicated` → `/dedicated` |
| **v1.2** | **2026-05-14** | **PRD-DAMYANG-001 + PRD-002 반영**. 화면 ID **ORG03-V → STF04-V**, URL `/organization/dedicated` → `/staff/dedicated`, 모듈 ORG → STF (안전보건 인력), SFR 매핑 SFR-006 → **SFR-006 + SFR-010 공동**. GNB 진입 경로 "전담조직 > 전담인력" → "기반 관리 > 안전보건 인력 > 부서별 전담인력". 컨텐츠 동일. |
