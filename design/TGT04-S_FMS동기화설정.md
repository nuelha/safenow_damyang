...
| 화면 ID | TGT04-S |
| 화면명 | FMS 동기화 설정 |
| URL | `/targets/fms_sync` |
| 화면 유형 | S (Settings, 관리자 전용) |
| 접근 권한 | CEO / GM / SHM |

## 2. 진입 경로

- GNB **관리대상 관리** > LNB **FMS 동기화 설정**
- 새올 포틀릿 알림 (FMS 동기화 실패·신규 시설 등록대기 등) 링크

> 모듈 내 다른 화면(관리대상 현황)으로의 이동은 좌측 사이드 메뉴(LNB)로 처리.
...
| v1.1 | 2026-05-12 | 모듈 탭 제거 → LNB 사이드 메뉴로 이동. URL `?tab=fms_sync` → `/fms_sync` |
